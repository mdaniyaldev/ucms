...
export async function getMyProfile() {
  ...
  if (error) {
    console.error("[faculty.getMyProfile] error:", error);
    throw error;
  }
...
export async function listDepartments() {
  ...
  if (error) {
    console.error("[faculty.listDepartments] error:", error);
    throw error;
  }
...
export async function listDepartmentsWithCategories() {
  ...
  if (error) {
    console.error("[faculty.listDepartmentsWithCategories] error:", error);
    throw error;
  }
...
export async function createComplaint({ title, body, category, departmentId }) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      student_id: user.id, // Using student_id as submitter reference
      status: "open",
    })
    .select("id")
    .single();
  ...
  if (error) {
    console.error("[faculty.createComplaint] error:", error);
    throw error;
  }
...
export async function listMyComplaints() {
  ...
  if (error) {
    console.error("[faculty.listMyComplaints] error:", error);
    throw error;
  }
...
export async function getMyComplaintById(complaintId) {
  ...
  if (error) {
    console.error("[faculty.getMyComplaintById] error:", error);
    throw error;
  }
...
export async function listMyNotifications({ onlyUnread = false, limit = 50 } = {}) {
  ...
  if (error) {
    console.error("[faculty.listMyNotifications] error:", error);
    throw error;
  }
...
export async function getUnreadNotificationCount() {
  ...
  if (error) {
    console.error("[faculty.getUnreadNotificationCount] error:", error);
    throw error;
  }
...
export async function markNotificationRead(notificationId) {
  ...
  if (error) {
    console.error("[faculty.markNotificationRead] error:", error);
    throw error;
  }
...
export async function markAllNotificationsRead() {
  ...
  if (error) {
    console.error("[faculty.markAllNotificationsRead] error:", error);
    throw error;
  }
...
export async function subscribeToMyComplaints(onChange) {
  ...
  const channel = supabase
    .channel(`faculty-complaints:${user.id}`)
    .on(
      "postgres_changes",
      {
        event: "UPDATE",
