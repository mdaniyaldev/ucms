...
import {
  createComplaint,
  listDepartmentsWithCategories,
} from "../../lib/faculty";
import { useNavigate } from "react-router-dom";

export function FacultySubmitComplaintForm({ language = "en" }) {
  ...
  const text = {
    en: {
      title: "Submit New Issue Report",
      subtitle: "Report departmental or administrative issues",
    ...
    ur: {
      title: "نیا مسئلہ رپورٹ کریں",
      subtitle: "محکمانہ یا انتظامی مسائل کی اطلاع دیں",
      complaintTitle: "مسئلے کا عنوان",
      titlePlaceholder: "اپنے مسئلے کا مختصر عنوان",
      description: "تفصیل",
  ...
  const categories = [
    ...
    { value: "transport", label: text[language].transport, icon: Bus },
    {
      ...
      label: text[language].administrative,
      icon: Building2,
    },
  ];
  ...
  const handleFileChange = (e) => {
    ...
    const videoCount = nextFiles.filter((file) =>
      file.type.startsWith("video/"),
    ).length;
    if (videoCount > 1) {
      setError("Only one video is allowed per complaint");
      e.target.value = "";
      return;
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                required
                className="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={5}
                required
                className="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  required
                  className="w-full rounded-lg border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2 text-gray-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <option value="">
              ...
              {uploadedFiles.length > 0 && (
                <div className="mt-4 space-y-2">
                  {uploadedFiles.map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-3 bg-gray-100 dark:bg-slate-800 rounded-lg"
                    >
